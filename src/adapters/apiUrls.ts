...
//
// 命名约定：
//   - 共用（跨端）：ACT_MIHOYO_BASE_URL / DEVICE_FP_URL / ROLE_URL
//   - 按游戏前缀：GI_ / HSR_ / ZZZ_
//   - _CALC_PAGE_URL：计算器页面 URL（同时用于 checkLogin 打开页面 与 getAccount 的 openUrl）
//   - ROLE_URL：绑定角色列表 API（三端统一，game_biz 由各端 GameApiConfig 提供）
//   - 其余：各游戏接口 URL
...
/** 绑定角色列表 API（三端统一，靠 game_biz 参数区分：GI=hk4e_cn / HSR=hkrpg_cn / ZZZ=nap_cn） */
export const ROLE_URL = "https://api-takumi.mihoyo.com/binding/api/getUserGameRolesByCookie";
